...
if (localStatus != 0)
{
    TErr(("sendMessage(msgId=%x,targetMsgId=%x)=%d", messageID, targetedMessageID, localStatus));
}
...
if (localStatus != 0)
{
    TErr(("receiveMessage(msgId=%x,targetMsgId=%x)=%d", messageID, targetedMessageID, localStatus));
}
